<?php
	...
	else
	{
		 if (isset($_POST['submitmr'])) {
		 	...
		 	$sql = "DELETE FROM `mess_request` WHERE `mess_request`.`req_id` = $req_id";
			if(!mysqli_query($conn, $sql))
		 		echo mysqli_error($conn);
		 }
		

		$sql = "SELECT * FROM `mess_request`";
...
<body>
	...
	<div class="limiter">
		...
		<form style = "margin-left: 38%;" action = "messrequest.php" method=POST>
			<input type="text" name="req_id" placeholder="Enter MR_ID" class = "submitbutton submitbutton1">
			<input type="submit" name="submitmr" value="Delete Entry" class="submitbutton submitbutton1">
		</form>
	</div>
		...
		<?php 
			...
			foreach($rows as $row){ 
				?>
				<tr>
									<td class="column1"><?php echo $row['req_id']; ?></td>
									<td class="column2"><?php echo $row['sEmail'] ;?></td>
									<td class="column2"><?php echo $row['mess'] ;?></td>
